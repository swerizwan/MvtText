...
def main():
    ...
    for key, item in all_metrics.items():
        mean, conf_interval = get_metric_statistics(np.array(item),
                                                    replication_times)
        if any(keyword in key for keyword in ["gt_R_precision_top_1", "gt_R_precision_top_2", "gt_R_precision_top_3", "gt_Diversity", "MultiModality"]):
            all_metrics_new[key] = mean
        if "FID" in key:
            all_metrics_new[key] = conf_interval
    all__metrics = {key.replace('gt_', ''): value for key, value in all_metrics_new.items()}
    print_table(f"Main Metrics", all__metrics)
